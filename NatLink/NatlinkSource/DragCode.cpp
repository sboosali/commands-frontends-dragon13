...
BOOL CDragonCode::isNatSpeakRunning()
{
	...
	HANDLE hSem = CreateSemaphore(
		NULL, 1, 1, 
		"Dragon NaturallySpeaking 1.0 -> Server Semaphore" );
	if( hSem == NULL )
	{
		return FALSE;
	}
...
SDATA makeEmptyGrammar()
{
	...
	DWORD dwStartWordSize = ( strlen( START ) + 1 );
	dwStartWordSize = ( dwStartWordSize + 3 ) & ~3;

	DWORD dwEmptyWordSize = ( strlen( "Empty" ) + 1 );
...
PyObject * CDragonCode::getCurrentUser()
{
	...
	DWORD dwLength = _MAX_PATH + 1;
	CPointerChar pUser = new char[ dwLength ];
	DWORD dwNeeded;
	rc = pISRSpeaker->Query( pUser, dwLength, &dwNeeded );
	if( rc == E_BUFFERTOOSMALL )
	{
		dwLength = dwNeeded + 1;
		pUser = new char[ dwLength ];
		rc = pISRSpeaker->Query( pUser, dwLength, &dwNeeded );
	}
...
char * packString( PCCHAR * ppWords, DWORD * pdwSize, DWORD * pdwCount )
{
	...
	const char * pszScript, PCCHAR * ppWords, const char * pszComment )
{
	...
	else
	{
		DWORD dwCount = 0;
		DWORD dwListSize = 0;
		char * pszList = packString( ppWords, &dwListSize, &dwCount );
